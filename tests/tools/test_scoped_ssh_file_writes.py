...
def test_profile_roots_write_patch_and_symlink_isolation(tmp_path, monkeypatch):
    ...
    with terminal_scope({'TERMINAL_FILE_WRITE_ROOTS': json.dumps([str(root)])}):
...
def test_invalid_roots_fail_closed(tmp_path, policy):
    ...
    with terminal_scope({'TERMINAL_FILE_WRITE_ROOTS': policy}):
...
def test_gateway_cwd_uses_profile_scope_not_ambient(monkeypatch):
    ...
    with terminal_scope({'TERMINAL_ENV': 'ssh', 'TERMINAL_CWD': '/remote/profile/workspace'}):
